...
<body topmargin="0" >
  ...
  <tr>
    ...
    <!-- 引入右部页面 -->
    <td width="88%" align="center"  background="images/5.jpg" style="background-repeat:repeat-x;"><jsp:include flush='true' page="../userpages/addbacklog.jsp"/></td>
  </tr>
</table>
</body>
...
function checkdate(){
	if ($("date").value.replace(/(^\s*)|(\s*$)/g, "").length == 0) {
		...
		return false;
	      ...
	      var time = new Date($("date").value).valueOf();
	      if(now>time){
	    	 $("datemes").innerHTML = "时间不能小于当前时间";
	          return false;
	      }else{
	    	  $("datemes").innerHTML = "";
	          return true;
